function [FAD] = Autofluorescence_450nm_FAD_Feb21(nphoton,resolution,lambda1,lambda2)
fprintf('\n');
...
model.G.mediaPropParams   = 500; % Cell array containing any additional parameters to be passed to the getMediaProperties function
...
%% Media Properties function
    function mediaProperties = mediaPropertiesFunc(wavelength,parameters)
        ...
        j=4;
        mediaProperties(j).name  = 'Epidermis-FAD'; %(thickness: https://www.ncbi.nlm.nih.gov/pubmed/14690333)
        B = 0;
        S = 0.75;
        W = 0.75;
        Me = 0.03;
        ...
        musp = musp500*(fray*(wavelength/500).^-4 + (1-fray)*(wavelength/500).^-bmie);
        X = [B*S B*(1-S) W Me]';
        a1=xlsread('FAD_Feb21.xlsx');
        b=a1(:,1)-parameters;
        c=find(b(:,1)==0);
        mediaProperties(j).mua = .3643; %.3643;%.3643; %Absorption Spectra- https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1269010/pdf/biochemj01096-0100.pdf %.479*2.3
        mediaProperties(j).mus = musp/(1-gg);
        mediaProperties(j).g   = gg;
        mediaProperties(j).n   = 1.3;
        mediaProperties(j).VHC = 3391*1.109e-3;
        mediaProperties(j).TC  = 0.37e-2;
        mediaProperties(j).Y    = .033*a1(c,3); %https://www.sciencedirect.com/science/article/pii/S0301010403004385
    end
end
