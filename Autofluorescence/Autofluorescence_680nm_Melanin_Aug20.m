function [Melanin] = Autofluorescence_450nm_Melanin_Feb21(nphoton,resolution,lambda1,lambda2)
...
%% Media Properties function
    function mediaProperties = mediaPropertiesFunc(wavelength,parameters)
        ...
        j=4;
        mediaProperties(j).name  = 'Epidermis-Melanin'; %(thickness: https://www.ncbi.nlm.nih.gov/pubmed/14690333)
        B = 0;
        S = 0.0;
        W = 0.0;
        Me = 1;%0.03;
        ...
        musp = musp500*(fray*(wavelength/500).^-4 + (1-fray)*(wavelength/500).^-bmie);
        X = [B*S B*(1-S) W Me]';
        a1=xlsread('Melanin_Feb3.xlsx');
        b=a1(:,1)-parameters;
        c=find(b(:,1)==0);
        mediaProperties(j).mua = 519*(wavelength/500)^-3*0.02*.25;%0.1*(MU*X+muabase);%2.303*.04*488.79;%1.7*10^12 * wavelength^(-3.48);%2.303*8156.43*.04;%2.303 * epsilon * c   OMLC: µa = 1.70x1012 nm-3.48 [cm-1] for skin 
        mediaProperties(j).mus = musp/(1-gg);
        mediaProperties(j).g   = gg;
        mediaProperties(j).n   = 1.3;
        mediaProperties(j).VHC = 3391*1.109e-3;
        mediaProperties(j).TC  = 0.37e-2;
        mediaProperties(j).Y    = .00056*a1(c,3); %.06 %measured .002 but 1/10 of melanosomes is melanin
    end
