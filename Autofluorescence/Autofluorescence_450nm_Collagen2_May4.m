function [Collagen] = Autofluorescence_450nm_Collagen2_March26(nphoton,resolution,lambda1,lambda2)
fprintf('\n');
...
data(counter,7)=model.FMC.mediaProperties_funcHandles(1).mua;
data(counter,8)=model.FMC.mediaProperties_funcHandles(2).mua;
data(counter,9)=model.FMC.mediaProperties_funcHandles(3).mua;
Collagen=data;
counter=counter+1;
...
j=4;
mediaProperties(j).name = 'Collagen';
B = 0.002;
S = 0.67;
W = 0.65;
Me = 0;
...
musp = musp500*(fray*(wavelength/500).^-4 + (1-fray)*(wavelength/500).^-bmie);
X = [B*S B*(1-S) W Me]';
a1=xlsread('Collagen_March26.xlsx');
%a1=xlsread('FAD_Feb21.xlsx');
%a1=xlsread('Lipofuscin_Feb21.xlsx');
%a1=xlsread('PPIX_Feb21.xlsx');
%a1=xlsread('VitaminA_Feb21.xlsx');
b=a1(:,1)-parameters; 
c=find(b(:,1)==0);
mediaProperties(j).mua  = 7.8131; %.0005
mediaProperties(j).mus  = musp/(1-gg);
mediaProperties(j).g    = gg;
mediaProperties(j).n    = 1.3;
mediaProperties(j).Y    = .05*a1(c,3); %https://www.sciencedirect.com/science/article/pii/0024320588903049 
