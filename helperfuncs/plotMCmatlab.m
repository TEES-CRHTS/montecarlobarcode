...
global P_flu_emit QY1 absorbedincident P_exc_abs detFraction P_in boundary test1 boundary1
...
if MCorFMC.boundaryType == 1
    ...
    boundary1=MCorFMC.NI_zpos*G.dx*G.dy;
    ...
    yh = y(end); % y high
    zl = z(1); % z low
    zh = z(end); % z highplotMCmatlab(model);
...
if ~isnan(MCorFMC.NFR(1))
    ...
    test1=100*G.dx*G.dy*G.dz*sum(sum(sum(mua_vec(MCorFMC.M).*MCorFMC.NFR)))/P_in;
