...
global P_flu_emit QY1 absorbedincident P_exc_abs detFraction P_in boundary
...
%   h_f= plotVolumetric1(11,G.x,G.y,G.z,model.FMC.beam.sourceDistribution,'MCmatlab_fromZero');
...
boundary=MCorFMC.NI_zneg*G.dx*G.dy;
...
%   if simFluorescence || isnan(MCorFMC.beam.beamType) || MCorFMC.beam.beamType ~= 2
...
%                 
%  
%   h_f= plotVolumetric1(3 + figNumOffset,G.x,G.y,G.z,mua_vec(MCorFMC.M).*MCorFMC.NFR,'MCmatlab_fromZero');
...
%   h_f= plotVolumetric1(4 + figNumOffset,G.x,G.y,G.z,MCorFMC.NFR,'MCmatlab_fromZero');
...
% 
   test=char(fluorescenceOrIncident);
if strcmp(test,'incident ')==1
    absorbedincident=G.dx*G.dy*G.dz*sum(sum(sum(mua_vec(MCorFMC.M).*MCorFMC.NFR)))/P_in;
end
...
if MCorFMC.nExamplePaths > 0
  ...
  h_f= plotVolumetric1(5 + figNumOffset,G.x,G.y,G.z,G.M_raw,'MCmatlab_GeometryIllustration',MCorFMC.mediaProperties_funcHandles);
...
if MCorFMC.useLightCollector
 ...
 % h_f= plotVolumetric1(6 + figNumOffset,G.x,G.y,G.z,G.M_raw,'MCmatlab_GeometryIllustration',MCorFMC.mediaProperties_funcHandles);
...
%     h_f= plotVolumetric1(7 + figNumOffset,G.x,G.y,G.z,MCorFMC.NFRdet,'MCmatlab_fromZero');
%                   
%  
...
%     title(['Normalized fluence rate of collected ' fluorescenceOrIncident 'light [W/cm^2/W.incident]'])
  ...
  if LC.res > 1 && ~simFluorescence && LC.nTimeBins > 0
    ...
    h_f= plotVolumetric1(8 + figNumOffset,LC.X,LC.Y,timevector,LC.image,'slicePositions',[1 1 0]);
