...
for r=1:3
    ...
    model.G.mediaPropParams   = 450; % Cell array containing any additional parameters to be passed to the getMediaProperties function
    ...
    for c=1:cmax
        ...
        detectord1(c,1,r)=lambda;
        detectord1(c,2,r)=absorbedincident*P_flu_emit/(P_exc_abs)*(detFraction/P_in)/100;
        %         datad(c,1)=lambda;
    ...
    % %% Post-processing
    % tau0=251; %lifetime with 0 quencher, us
    % ksv=.29; %Stern Volmer constant = Dynamic Constant * tau0 (uM^-1 * conc)
    % k=10;
    % conc=0;%linspace(1,150,k); %uM oxygen or glucose
    % t=linspace(0,100,k); %microseconds
    %     for i=1:k
    %         %dat1(:,:,:,i)=dat.*exp((-t(i))/(tau0 / (1+ksv*conc)));
    %         %Ldat1(:,:,:,i)=log10(dat1(:,:,:,i));
    %          detector(c,i+1,1,r)=(absorbedincident)*(P_flu_emit/P_exc_abs)*(detFraction/P_in)/100*exp((-t(i))/(tau0 / (1+ksv*conc)));
    %     end
    % clearvars P_flu_emit P_exc_abs detFraction P_in
...
mediaProperties(j).mua   = 1e-5;%1e-6; %10
mediaProperties(j).mus   = 1e-5;%.0005;
mediaProperties(j).g     = .75;
mediaProperties(j).n     = 1.4;

j=4;
mediaProperties(j).name  = 'FRET Sensing Chemistry';
...
j=5;
a1=xlsread('PdTCPP2.xlsx');
b=a1(:,1)-parameters;
c=find(b(:,1)==0);
mediaProperties(j).name  = 'PdTCPP';
mediaProperties(j).mua   = .0066;%.173; %(nm (M-1 cm-1))630nm
mediaProperties(j).mus   = 1e-6;
mediaProperties(j).g     = .75;
mediaProperties(j).n     = 1.4;
mediaProperties(j).Y     =.068*a1(c,3);%*a1(c,3);%.21= quantum yield
