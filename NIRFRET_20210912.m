...
for iter=1:1
    ...
    cmax=80;
    for c=1:cmax
        lambda=round(656+(c/cmax*209));
        if lambda>656 && lambda<842
            %% Donor
            %% G
    ...
    for c=1:cmax
        lambda=round(663+(c/cmax*209));
        if lambda>722 && lambda<872
            model.G.silentMode        = false; % (Default: false) Disables command window text and progress indication
    ...
    for c=1:cmax*1.33
        ...
        if lambda>722 && lambda<872
            ...
            model.G.mediaPropParams   = lambda; % Cell array containing any additional parameters to be passed to the getMediaProperties function
            model.G.mediaPropertiesFunc = @mediaProp_FRET3; % Media properties defined as a function at the end of this file
            model.G.geomFunc          = @GeometryDefinition_FRET3; % Function to use for defining the distribution of media in the cuboid. Defined at the end of this m file.
            model.G.geomFuncParams    = {0.03}; % Cell array containing any additional parameters to pass into the geometry function, such as media depths, inhomogeneity positions, radii etc.
            ...
            detectoraf1(c,2)=absorbedincident*P_flu_emit/(P_exc_abs)*(detFraction/P_in)/100;
            detectoraf1(c,1)=lambda;
            dataa(c,1)=lambda;
            dataa(c,2)=absorbedincident;
            dataa(c,3)=P_flu_emit/(P_exc_abs);
            dataa(c,4)=(detFraction/P_in)/100;
        ...
        r0= 6.8; %nm, forster radius, VIS fret is 6
        r=3.5;%*zztop/10;
        E=(r0^6/(r0^6+r^6));
        r1=6;
        Eu=(r0^6/(r0^6+r1^6));
        ...
        for i=1:size(detectord1,1)
            Detectord(i,2)=detectord1(i,2).*(1-spectrumoverlap*E*.023*.932*3/2);
            Detectord(i,1)=detectord1(i,1);
        end
        
        
        for i=1:size(detectora1,1)
            Detectora(i,2)=detectora1(i,2)*spectrumoverlap*E*1*.023*3/2*10;
        ...
        Detectoraf=LinearInterpolator(detectoraf1,0);
        Detectoraa(:,2)=Detectoraa(:,2)+Detectoraf(:,2);
        ...
        %
        for i=1:size(Detector4,1)-1
            if Detector4(i,1)==Detector4(i+1,1)
                Detector4(i+1,:)=[];
            end
        end
        Detector5(:,1,iter)=Detector4(:,1);
        Detector5(:,2,iter)=Detector4(:,2);
        
        a(:,1)=Detector4(:,1);
        a(:,iter+1)=Detector4(:,2);
...
function M = GeometryDefinition_FRET3(X,Y,Z,parameters)
M=2*ones(size(X));
M(Z<.01)=1;
M(Z>.16)=6;
M(X>-.325 & X<.325 & Y>-.035 & Y<.035 & Z>.155 & Z<.245)=3;
...
j=4;
a2=xlsread('Cy5.5_2.xlsx');
b1=a2(:,1)-parameters;
c1=find(b1(:,1)==0);
%Measuring the FRET response from the FRET LED- calculating FRET
mediaProperties(j).name  = 'FRET Chemistry';%'FRET';
mediaProperties(j).mua   = 25; 
...
function mediaProperties = mediaProp_FRET3(wavelength,parameters)
load spectralLIB.mat
MU(:,1) = interp1(nmLIB,muaoxy,wavelength);
MU(:,2) = interp1(nmLIB,muadeoxy,wavelength);
MU(:,3) = interp1(nmLIB,muawater,wavelength);
...
c2=find(b2(:,1)==0);
%Measuring the FRET response from the FRET LED- calculating FRET
mediaProperties(j).name  = 'FRET';%'FRET';
mediaProperties(j).mua   = 25; 
mediaProperties(j).mus   = 1e-6;
mediaProperties(j).g     = .75;
mediaProperties(j).n     = 1.3;
mediaProperties(j).Y     =a3(c2,3)*0.3;%

j=5;
mediaProperties(j).name  = 'BMAP';%'PdTPTBP';
mediaProperties(j).mua   = 168;%.173; %(nm (M-1 cm-1))630nm
mediaProperties(j).mus   = 1e-6;
mediaProperties(j).g     = .75;
mediaProperties(j).n     = 1.4;
